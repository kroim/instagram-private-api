        ...
        .then(function(response){
            ...
            switch(json.step_name){
                case 'select_verify_method':{
                    // console.log("switch case defaultMethod === ", defaultMethod);
                    return new WebRequest(session)
                        .setMethod('POST')
                        .setUrl(that.apiUrl)
                        .setHeaders({
    ...
    function parseResponse(response){
        ...
        switch(challenge.challengeType){
            case 'SelectVerificationMethodForm':{
                // console.log("SelectVerificationMethodForm ...");
                return new WebRequest(session)
                    .setMethod('POST')
                    .setUrl(checkpointError.url)
                    .setHeaders({
            ...
            case 'VerifyEmailCodeForm':{
                console.log("VerifyEmailCodeForm ...");
                return new EmailVerificationChallenge(session, 'email', checkpointError, json);
            }
            case 'VerifySMSCodeForm':{
                console.log('VerifySMSCodeForm');
                return new PhoneVerificationChallenge(session, 'phone', checkpointError, json);
            }
...
var EmailVerificationChallenge = function(session, type, checkpointError, json) {
    this.submitEmail = json.step_name === "verify_email";
    Challenge.apply(this, arguments);
}
//Confirming email address.
//We need to return EmailVerificationChallenge that can be able to request code.
//So, if we need to submit email address first - let's do it. If not - just return current EmailVerificationChallenge;
EmailVerificationChallenge.prototype.email = function(email, code) {
    var that = this;
    if (!this.submitEmail) return Promise.resolve(this);
    let instaEmail = (that.json && that.json.step_data) ? that.json.step_data.contact_point : null;
    let _email = email || instaEmail;
    if(!_email) return new Error('Invalid Email Address');
    ...
    return new WebRequest(that.session)
        ...
        .then(function(response){
            ...
            return new EmailVerificationChallenge(that.session, 'email', that.error, json);
        })
}
